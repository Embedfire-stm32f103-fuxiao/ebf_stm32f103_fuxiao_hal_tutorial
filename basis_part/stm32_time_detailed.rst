...
STM32F1xx系列控制器有2个高级控制定时器、4个通用定时器和2个基本定时器，还有2个看门狗定时器。
...
我们一般设置该位为默认值 0，即 TIMxCLK 为总线时钟的两倍，使得表 32-1 中可选的最大定时器时钟为 72MHz，
即基本定时器的内部时钟(CK_INT)频率为 72MHz。
...
预分频器的输入时钟 CK_PSC 为 72MHz，所以设置预分频器值为(7200-1)即可满足。
...
初始化结构体定义在 stm32f1xx_hal_tim.h 文件中，
初始化库函数定义在 stm32f1xx_hal_tim.c 文件中，编程时我们可以结合这两个文件内注释使用。
...
(7200-1)，基本定时器使能内部时钟，频率为 84MHz，经过预分频器后得到 10KHz 的频率。
...
STM32F103VET6 的高级/通用定时器的 IO 分配具体见下图。
配套开发板因为 IO 资源紧缺，定时器的 IO 很多已经复用它途，故下表中的 IO 只有部分可用于定时器的实验。

========  =============  ========  ============  ========  ===========  ========  
   \        **TIM1**     **TIM8**    **TIM2**    **TIM3**   **TIM4**    **TIM5**  
========  =============  ========  ============  ========  ===========  ========  
**CH1**   PA8/PE9        PC6       PA0/PA15      PC6       PD12/PB6     PA0 
**CH2**   PE11/PA9       PC7       PA1/PB3       PC7       PD13/PB7     PA1
...
由于ETRP的信号的频率不能超过TIMx_CLK（72M）的1/4，当触发信号的频率很高的情况下，
...
初始化结构体定义在stm32f1xx_hal_tim.h和stm32f1xx_hal_tim_ex.h文件中，
初始化库函数定义在stm32f1xx_hal_tim.c和stm32f1xx_hal_tim_ex.c文件中，编程时我们可以结合这四个文件内注释使用。
